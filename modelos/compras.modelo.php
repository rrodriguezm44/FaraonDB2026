...
//    }
    ...
    static public function mdlRegistrarCompra($formulario_compra, $detalle_compra, $total_descuento, $total)
    {
        // Log para debugging
        error_log('mdlRegistrarCompra called');
        error_log('Formulario compra: ' . print_r($formulario_compra, true));
        error_log('Detalle compra: ' . print_r($detalle_compra, true));
        error_log('Total descuento: ' . $total_descuento);
        error_log('Total: ' . $total);

        $dbh = Conexion::conectar();

        // Validar que nro_boleta exista
        $nro_boleta = isset($formulario_compra["iptNroVenta"]) ? trim($formulario_compra["iptNroVenta"]) : null;
        
        if (empty($nro_boleta)) {
            $respuesta["tipo_msj"] = "error";
            $respuesta["msj"] = "El número de boleta/venta es obligatorio";
            return $respuesta;
        }

        // Validar que cliente_id sea válido (no sea 0 o vacío)
        $cliente_id = isset($formulario_compra["selCliente"]) ? intval($formulario_compra["selCliente"]) : 0;
        
        if ($cliente_id <= 0) {
            $respuesta["tipo_msj"] = "error";
            $respuesta["msj"] = "Debe seleccionar un cliente válido";
            return $respuesta;
        }

        // Validar que vendedor_id sea válido (no sea 0 o vacío)
        $vendedor_id = isset($formulario_compra["selVendedor"]) ? intval($formulario_compra["selVendedor"]) : 0;
        
        if ($vendedor_id <= 0) {
            $respuesta["tipo_msj"] = "error";
            $respuesta["msj"] = "Debe seleccionar un vendedor válido";
            return $respuesta;
        }
        ...
        if (intval($existe["existe"]) > 0) {
            $respuesta["tipo_msj"] = "error";
            $respuesta["msj"] = "El comprobante de venta: " . $nro_boleta . ' ya fue registrado';
            return $respuesta;
        }
        ...
        try {
            ...
            $stmt->execute(array(
                ...
                ':total' => $total,
                ':id_cliente' => $formulario_compra["selCliente"],
                ':obs_venta' => $formulario_compra["iptObservacion"],
                ':codVendedor' => $formulario_compra["selVendedor"],
                ':tipoPago' => $formulario_compra["selTipoPago"],
                ':docVenta' => $formulario_compra["selDocumentoVenta"],
                ':usuarioID' => $formulario_compra["codUsuario"],
                ':fecha_entrega' => $formulario_compra["iptFechaEntrega"]
            ));

            $id_venta_inserted = $dbh->lastInsertId();
            $dbh->commit();

            // Actualizar correlativo de ventas en la empresa
            ...
            for ($i = 0; $i < count($detalle_compra); $i++) {
                 ...
                 ****************************************************************************/
                // Comentado temporalmente: procedimiento no existe en la BD
                
                $concepto = 'VENTA';

                $stmt = Conexion::conectar()->prepare("call prc_registrar_kardex_ventas_adquisicion (
                                                                                        :codigo_producto,
                                                                                        upper(:concepto),
                                                                                        upper(:comprobante),
                                                                                        :cantidad_venta,
                ...
                $stmt->execute(array(
                    ...
                    ':codigo_producto' => $detalle_compra[$i]->codigo_producto,
                    ':concepto' =>  $concepto,
                    ':cantidad_venta' => $detalle_compra[$i]->cantidad_temp,
                    ':precio_venta' => $detalle_compra[$i]->precio_venta_temp
                ));
                $dbh->commit();
            ...
            $respuesta["msj"] = "Se registró la venta correctamente con Nro: " . $id_venta_inserted;
        } catch (Exception $e) {
            $dbh->rollBack();
            $respuesta["tipo_msj"] = "error";
            $respuesta["msj"] = "Error al registrar la venta correctamente " . $e->getMessage();
        }
